...
namespace Tank5
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            if (ComboBoxRusultTextBlock == null) return;

            var combo = (ComboBox)sender;
            var item = (ComboBoxItem)combo.SelectedItem;
            ComboBoxRusultTextBlock.Text = item.Content.ToString();
            ...
            var selectedItems = MyListBox.Items.Cast<ListBoxItem>()
                .Where(p => p.IsSelected)
                   .Select(t => t.Content.ToString())
                      .ToArray();
            ListBoxRusultTextBlock.Text = string.Join(",", selectedItems);
        }

        private void MyToggleButton_Click(object sender, RoutedEventArgs e)
        {
            ToggleButtonRusultTextBlock.Text = MyToggleButton.IsChecked.ToString();
        }
    }
}
